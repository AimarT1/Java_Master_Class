...
public class SearchTree implements NodeList {
    ...
    public boolean addItem(ListItem newItem) {
        ...
        ListItem cuurentitem = this.root;
        while (cuurentitem != null){
            int compare = (cuurentitem.compareTo(newItem));
            if (compare < 0 ){
                //new item is greater, move right if posiible
                if (cuurentitem.next() != null){
                    cuurentitem = cuurentitem.next();
                }else{
                    //theres no node to the right, so just add at this point
                    cuurentitem.setNext(newItem);
                    return true;
                }
            }else if (compare > 0){
                // new item is less move left if posiible
                if (cuurentitem.previous() != null){
                    cuurentitem = cuurentitem.previous();
            }else {
                //theres no node to the left, so add at this point
                cuurentitem.setPrevious(newItem);
                return true;
            }
        }else{
            //equal so dont add
        ...
        //we cant actually get here but java complains if there is no return.
        return false;
    }

    @Override
    //this method is only determening the items to be removed.
    ...
    //actual code to remove the item
